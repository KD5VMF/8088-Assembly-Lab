...
;   - Keeps REV15/REV18 table layout.
...
;       OPS, KOPS, or MOPS.
;   - Spinner/art stays close to the RATE unit text.
;   - BAR and DRK are randomized independently from 1 to 100.
;   - GAS is randomized independently from 1 to 10.
;   - BAR/DRK/GAS refresh automatically after every 25 main
;     display iterations, giving the live galaxy changing inputs.
;
; Build:
;   TASM Galaxy16
;   TLINK Galaxy16
...
DRAW_ONE_ROW PROC
    ; REV20 keeps REV15/REV18 table alignment and refreshes randomized mass fields.
...
; ================================================================
; SIMPLE STARTUP RANDOM NUMBER GENERATOR
; ================================================================
; The seed is initialized once from the BIOS clock tick counter.
; BAR, DRK, and GAS are generated independently for every ring.
; After startup, the same pseudo-random stream is used again every
; 25 main display iterations to refresh BAR/DRK/GAS.

INIT_RNG_FROM_BIOS PROC
    push ax
    push cx
    push dx
...
RNG_SEED_OK:
    ...
    pop ax
    ret
INIT_RNG_FROM_BIOS ENDP

RANDOM16 PROC
    ; Output: AX = next pseudo-random 16-bit value.
...
RAND_1_TO_10 PROC
    ; Output: AX = random integer from 1 to 10.
...
RAND_1_TO_100 PROC
    ; Output: AX = random integer from 1 to 100.
...
UPDATE_GALAXY_RANDOMS_25 PROC
    ; Refresh BAR/DRK/GAS after every 25 main display iterations.
    ; This keeps the galaxy alive without changing the radius layout.

    inc word ptr [MASS_REFRESH_COUNT]
    cmp word ptr [MASS_REFRESH_COUNT], 25
    jb UPDATE_RANDOMS_DONE

    mov word ptr [MASS_REFRESH_COUNT], 0
    call REFRESH_GALAXY_MASSES

UPDATE_RANDOMS_DONE:
    ret
UPDATE_GALAXY_RANDOMS_25 ENDP

REFRESH_GALAXY_MASSES PROC
    ; Re-roll only the live mass components.
    ; BAR = independent 1..100
    ; DRK = independent 1..100
    ; GAS = independent 1..10
...
INIT_RING_LOOP:
    ...
    ; BAR_M10 = random 1..100 for this ring.
    call RAND_1_TO_100
    mov [BAR_M10 + si], ax

    ; DARK_M10 = independent random 1..100 for this ring.
    call RAND_1_TO_100
    mov [DARK_M10 + si], ax

    ; GAS_M10 = independent random 1..10 for this ring.
    call RAND_1_TO_10
    mov [GAS_M10 + si], ax
...
TITLE_STR       db 'GALAXY MATH 88 REV20',0
